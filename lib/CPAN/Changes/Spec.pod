...
=head1 SYNOPSIS
    ...
    Revision history for perl module Foo::Bar with optional hints
...
A hint is a simple optional text hinting at whether an update is necessary, it 
can be one of following two values:

=over

=item * Update recommended

For releases, which address serious issues/bugs like security fixes, memory leaks etc.

=item * Update not required

For releases which are housekeeping releases, updates to tests, documentation, build scripts etc.
...
The update hint in the release heading line is optional and it is a basic 
textual extension to the required format (see above):

    <Version>(whitespace/non-"word" characters)<Date> -  <Hint>
    (whitespace)<Change>
