...
optional.
    ...
    <Prefix><Version>(whitespace/non-"word" characters)<Date>
    (whitespace)<Change>
...
A C<Prefix> is optional and is ignored if present. The only supported prefix
is /=head\d\s+/ to allow Changes files to be formatted using L<perlpod>.
